...
import PasswordLevel from "../types/password-level";
...
const countPaswordLevel = (number: string): number => {
  let strengths = 0;
...
export  const getStrengthPassword = (password: string): PasswordLevel => {
  const count = countPaswordLevel(password);
